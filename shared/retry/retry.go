...
func WithBackoff(ctx context.Context, cfg Config, operation func() error) error {
	...
	for attempt := 0; attempt <= cfg.MaxRetries; attempt++ {
		if attempt > 0 {
			logger.LogWarn(ctx, "Retry attempt %d/%d after %v", attempt, cfg.MaxRetries, wait)

			select {
			// Ensures backoff is compatible with goroutine channels
			case <-ctx.Done():
				return ctx.Err()
		...
		logger.LogWarn(ctx, "Operation failed (attempt %d/%d): %v", attempt+1, cfg.MaxRetries, err)
	}
